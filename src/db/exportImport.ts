...
function validateBackup(data: Row): DatabaseExport {
  ...
  const sources = tableRows.sources.map((r, i): Source => { const path=`sources[${i}]`; const rawText=text(r,'rawText',path); if (new TextEncoder().encode(rawText).byteLength > MAX_SOURCE_TEXT_BYTES) fail(`${path}.rawText exceeds the 2 MB limit.`); safeJson(r.metadata,`${path}.metadata`); return { id:text(r,'id',path,false), projectId:text(r,'projectId',path,false), participantId:text(r,'participantId',path,false), segmentId:nullableText(r,'segmentId',path), date:number(r,'date',path), type:enumValue(r,'type',path,['interview','email','survey','sales_call','support','observation','other']), rawText, metadata:r.metadata as Record<string, unknown>, tags:stringArray(r,'tags',path) }; });
  ...
  evidenceSignals.forEach(x=>{const s=sourceMap.get(x.sourceId); if(!s || s.projectId!==x.projectId) fail(`evidence "${x.id}" has an invalid or cross-project source.`); if(x.segmentId && segmentMap.get(x.segmentId)?.projectId!==x.projectId) fail(`evidence "${x.id}" has an invalid or cross-project segment.`); if(x.hypothesisId && hypothesisMap.get(x.hypothesisId)?.projectId!==x.projectId) fail(`evidence "${x.id}" has an invalid or cross-project hypothesis.`);});
